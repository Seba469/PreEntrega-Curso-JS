...
function pestañaCarrito (){
    ...
    for(let producto of productosCarrito){
        ...
        `
            ...
            <td><button id="${producto.id}" class="btn btnBorrar">Borrar</button></td>
        `
        tabla.append(fila)
    }
    ...
    btnBorrar.forEach(boton => {
        boton.onclick = () =>{
            let borrarProducto = productosCarrito.findIndex(prod => prod.id === parseInt(boton.id))
            
            if (productosCarrito[borrarProducto].cantidad > 1){
                productosCarrito[borrarProducto].cantidad --
            }else{
                delete(productosCarrito[borrarProducto])
            }
